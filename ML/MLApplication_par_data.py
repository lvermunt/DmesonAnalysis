...
run: python MLApplication_par_data.py cfgFileNameML.yml
'''
import os
import sys
import argparse
...
def appl(inputCfg, PtBin, OutPutDirPt, ModelHandl, DataDfPtSel):
    ...
    #print(ModelHandl.get_training_columns())
    #print(ModelHandl.get_model_params())
    #print(ModelHandl.get_model_module())
    #print(ModelHandl.get_n_classes())
...
def main(): #pylint: disable=too-many-statements
    ...
    if not isinstance(inFolderNameDataPar, list):
        ...
        for il in range(len(inFolderNameDataPar)):
            childname_listlist = []
            for subdirname in os.listdir(inFolderNameDataPar[il]):
                if os.path.isdir(f'{inFolderNameDataPar[il]}/{subdirname}'):
                    childname_listlist.append(f'{subdirname}/')
            childname_list.append(childname_listlist)

        for il in range(len(inFolderNameDataPar)):
            for childname in childname_list[il]:

                #if childname not in ['child_22', 'child_23', 'child_24', 'child_25', 'child_26', 'child_27', 'child_28', 'child_29', 'child_30', 'child_31', 'child_32', 'child_33', 'child_34', 'ch$
                #    print(f'Not running over {childname} for the moment (only for this test)')
                #    continue

                inFileNamesData = []
                for dirpath, dirnames, filenames in os.walk(f'{inFolderNameDataPar[il]}/{childname}'):
                    for filename in [f for f in filenames if f.endswith(inFileNameData)]:
                        inFileNamesData.append(os.path.join(dirpath, filename))
                ...
                for iBin, PtBin in enumerate(PtBins):
                    ...
                    OutPutDirPt = os.path.join(os.path.expanduser(inputCfg['output']['dir'][il]), f'{childname}/pt{PtBin[0]}_{PtBin[1]}')
                    if os.path.isdir(OutPutDirPt):
                        print((f'\033[93mWARNING: Output directory \'{OutPutDirPt}\' already exists,'
                               ' overwrites possibly ongoing!\033[0m'))
                    else:
