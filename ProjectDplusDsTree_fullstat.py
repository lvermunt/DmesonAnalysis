...
if isMC:
    ...
    inFileNamePrompt = inputCfg['tree']['filenamePrompt']
    inFileNamesPrompt = []
    inFolderNamePromptPar = inputCfg['tree'].get('prompt_foldername', None)
    ...
    for fileprompt in inFileNamesPrompt:
        df_ = LoadDfFromRootOrParquet(fileprompt, inputCfg['tree']['dirname'], inputCfg['tree']['treename'])
        arr_df_prompt.append(df_)
    dataFramePrompt = pd.concat(arr_df_prompt)
    ...
    #Get filename. This parallel script only works in case 'foldername' is also provided
    inFileNameFD = inputCfg['tree']['filenameFD']
    inFileNamesFD = []
    inFolderNameFDPar = inputCfg['tree'].get('FD_foldername', None)
    ...
    for filefd in inFileNamesFD:
        df_ = LoadDfFromRootOrParquet(filefd, inputCfg['tree']['dirname'], inputCfg['tree']['treename'])
        arr_df_fd.append(df_)
    dataFrameFD = pd.concat(arr_df_fd)
    ...
    #Get filename. This parallel script only works in case 'foldername' is also provided
    inFileNameData = inputCfg['tree']['data']
    inFileNamesData = []
    inFolderNameDataPar = inputCfg['tree'].get('data_foldername', None)
    if inFolderNameDataPar is not None:
        if isinstance(inFolderNameDataPar, list):
            if isinstance(inFileNameData, list):
                for il in range(len(inFolderNameDataPar)):
                    for dirpath, dirnames, filenames in os.walk(inFolderNameDataPar[il]):
                        for ifil in range(len(inFileNameData)):
                            for filename in [f for f in filenames if f.endswith(inFileNameData[ifil])]:
